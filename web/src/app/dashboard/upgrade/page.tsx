...
export default function UpgradePage() {
  ...
  useEffect(() => {
    if (searchParams.get('success') === 'true') {
      setNotification({ type: 'success', msg: '🎉 Upgrade realizado com sucesso! Seu plano foi atualizado.' })
    }
    if (searchParams.get('canceled') === 'true') {
      setNotification({ type: 'error', msg: 'Checkout cancelado. Nenhuma cobrança foi feita.' })
    }
  }, [searchParams])
  ...
  async function handleCheckout(planId: string) {
    ...
    try {
      ...
      setNotification({ type: 'error', msg: 'Erro ao conectar com o Stripe' })
    }
      ...
      <div className={styles.content}>
        ...
        {notification && (
          <div style={{
            ...
            border: `1px solid ${notification.type === 'success' ? 'rgba(0,255,136,0.3)' : 'rgba(255,68,68,0.3)'}`,
            borderRadius: '6px', padding: '12px 16px',
            fontFamily: 'var(--font-ui-mono)', fontSize: '13px',
            color: notification.type === 'success' ? 'var(--accent)' : '#ff4455',
            marginBottom: '24px',
              ...
              <div key={plan.id} style={{
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    } else if (!isCurrent && plan.price && plan.price > 0) {
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    fontFamily: 'var(--font-ui-mono)', fontSize: '11px',
                ...
                >
                  {isLoading ? 'Abrindo checkout...' :
                    isCurrent ? 'Plano atual' :
                    plan.id === 'enterprise' ? 'Falar conosco' :
                    'Fazer upgrade'}
                </button>
              </div>
            )
